...
import {api} from '../../api';
import {selectTopicFormData, topicApi} from '../topic';
...
describe('topic selectors', () => {
    ...
    test('selectTopicFormData derives editable values for auto-partitioned topics', async () => {
    ...
    test('selectTopicFormData falls back for invalid numeric backend values', async () => {
        const result = await seedTopicResult({
            self: {name: 'topic-b'},
